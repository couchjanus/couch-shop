...
## Deploy a Laravel App into Heroku

```bash
heroku login

heroku create couch-shop

heroku buildpacks:add heroku/php --app couch-shop

heroku buildpacks:add heroku/nodejs --app couch-shop
...
```bash
heroku addons:create heroku-postgresql:hobby-dev --app couch-shop
...
```bash
heroku config:set --app couch-shop APP_KEY=$(php artisan --no-ansi key:generate --show)
